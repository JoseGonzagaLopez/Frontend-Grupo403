...
const R      = 155;  // radius — wide enough for 6 items
const ARC0   =  18;  // start angle (deg from +X)
const ARC1   =  90;  // end angle (vertical up)
const T_SIZE =  58;
const I_SIZE =  50;
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            borderRadius: "50%",
            border: "1px dashed rgba(79,209,197,0.20)",
            pointerEvents: "none",
            opacity: open ? 1 : 0,
            transform: open ? "scale(1)" : "scale(0.55)",
            transition: reduced ? "none" : `opacity 380ms ${smooth}, transform 440ms ${spring}`,
          }}
        />
          ...
          return (
            <Link
              ...
              style={{
                ...
                opacity: open ? 1 : 0,
                transform: open
                  ? `translate(${x}px,${y}px) scale(${isHov ? 1.15 : 1})`
                  : "translate(0,0) scale(0.1)",
                transition: reduced ? "none" : open
                  ? `opacity 400ms ${spring} ${oDelay}ms, transform 460ms ${spring} ${oDelay}ms, background 160ms ${smooth}, border-color 160ms ${smooth}, box-shadow 160ms ${smooth}, color 160ms ${smooth}`
                  : `opacity 200ms ${smooth} ${cDelay}ms, transform 220ms ${smooth} ${cDelay}ms, background 160ms ${smooth}, border-color 160ms ${smooth}, box-shadow 160ms ${smooth}, color 160ms ${smooth}`,
                pointerEvents: open ? "auto" : "none",
              }}
            >
              {item.icon}
              <span
                ...
                style={{
                  ...
                  transform: `translateY(-50%) scale(${isHov ? 1 : 0.75})`,
        ...
        <button
          ...
          style={{
            ...
            transition: reduced ? "none"
              : `transform 500ms ${spring}, border-color 280ms ${smooth}, background 280ms ${smooth}, box-shadow 280ms ${smooth}`,
          }}
        >
          {!open && (
            <span
          ...
          <Image
            ...
            style={{
              ...
              transition: reduced ? "none" : `transform 500ms ${spring}`,
            }}
