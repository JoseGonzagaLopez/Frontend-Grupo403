...
import { Home, CalendarPlus, CalendarDays, X } from "lucide-react";
    ...
    <aside className={`sellix-sidebar sellix-sidebar--cliente${isOpen ? " sellix-sidebar--mobile-open" : ""}`}>
      ...
      <div className="sellix-sidebar__footer" style={{ paddingBottom: "70px" }}>
        <div className="sellix-sidebar__user">
          <div className="sellix-sidebar__user-avatar">C</div>
          <div className="sellix-sidebar__user-info">
            <span className="sellix-sidebar__user-name">Cliente</span>
            <span className="sellix-sidebar__user-role">Portal Cliente</span>
          </div>
        </div>
      </div>

      <style>{`
        .sellix-sidebar { position:fixed; left:0; top:0; bottom:0; width:var(--sidebar-width,240px); z-index:50; display:flex; flex-direction:column; padding:20px 12px; background:var(--surface,rgba(18,20,40,0.72)); backdrop-filter:blur(28px) saturate(180%); -webkit-backdrop-filter:blur(28px) saturate(180%); border-right:1px solid var(--border,rgba(99,102,241,0.18)); box-shadow:4px 0 32px rgba(0,0,0,0.18),inset -1px 0 0 rgba(255,255,255,0.04); transform: translateX(-100%); transition: transform 320ms cubic-bezier(0.25, 0.46, 0.45, 0.94); }
        .sellix-sidebar--mobile-open { transform: translateX(0); }
        ...
        .sellix-sidebar__close { display:flex; padding:4px; border-radius:6px; color:var(--text-secondary); background:none; border:none; cursor:pointer; margin-left:auto; transition:background 150ms ease,color 150ms ease; }
        ...
        .sellix-sidebar__user-avatar { width:32px; height:32px; border-radius:50%; background:var(--accent); color:#fff; font-size:0.75rem; font-weight:700; display:flex; align-items:center; justify-content:center; flex-shrink:0; box-shadow:0 2px 8px var(--accent-glow); }
