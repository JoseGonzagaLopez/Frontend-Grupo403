...
// Solo pending y confirmed son editables/borrables
...
export default function NegocioReservasClient({
  ...
  const filtered = useMemo(() => {
    ...
    if (filterService.trim())
      list = list.filter((a) =>
        a.serviceName?.toLowerCase().includes(filterService.toLowerCase())
      );
    if (sortOrder === "asc")
      list.sort(
        (a, b) =>
  ...
  async function quickStatusChange(id: number, status: ExtendedStatus) {
    setLoading(true);
    try {
      const updated = await updateAppointment(id, { status } as any);
      setAppointments((prev) => prev.map((a) => (a.id === id ? updated : a)));
      setSuccess(`Estado actualizado a "${STATUS_LABELS[status]}".`);
      setTimeout(() => setSuccess(""), 3000);
    } catch {
        ...
        >
          <div className="modal-card">
            ...
            <div className="modal-actions">
              <button className="secondary-btn" onClick={() => setDeleteTargetId(null)}>
                Cancelar
              </button>
              <button
                  ...
                  <tr key={a.id}>
                    ...
                    <td>
                      <span
                        ...
                        style={{
                          background:
                            STATUS_COLORS[a.status as ExtendedStatus] + "22",
                          color: STATUS_COLORS[a.status as ExtendedStatus],
                          border: `1px solid ${
                            STATUS_COLORS[a.status as ExtendedStatus]
                          }44`,
                    ...
                    <td>
                      {isEditable(a.status) ? (
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          {/* Pendiente: solo confirmar, editar y borrar */}
                          {a.status === "pending" && (
                            <button
                              className="secondary-btn"
                          ...
                          {/* Confirmada: marcar como terminada o no presentada */}
                          {a.status === "confirmed" && (
                            <>
                              <button
                                className="secondary-btn"
                                ...
                                onClick={() => quickStatusChange(a.id, "completed")}
                                disabled={loading}
                              ...
                              <button
                                ...
                                onClick={() => quickStatusChange(a.id, "no_show")}
                                disabled={loading}
