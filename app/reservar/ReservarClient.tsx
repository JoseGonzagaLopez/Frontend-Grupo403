...
function SearchableSelect({
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ color: value === "" ? "var(--text-tertiary)" : "inherit" }}>
...
export default function ReservarClient({
  ...
  const businessOptions = initialBusinesses.map((b) => ({
    id: b.id,
    ...
    return (
      <div className="public-page min-h-screen flex items-center justify-center p-4">
        <div className="public-card text-center flex flex-col items-center max-w-md w-full">
          <div className="public-success-icon mb-6">
            <CheckCircle2 size={64} color="var(--success-text)" />
          </div>
          <h2 className="text-2xl font-bold mb-2">¡Reserva Confirmada!</h2>
          <p className="text-var(--text-secondary) mb-8">
            Tu reserva ha sido registrada correctamente. Te contactaremos pronto para más detalles.
          </p>
          <div className="flex flex-col gap-3 w-full">
            <div className="p-4 bg-var(--surface-2) rounded-lg text-left">
              ...
              <p><strong>Fecha:</strong> {new Intl.DateTimeFormat("es-ES", {
                ...
                month: "2-digit",
                year: "numeric",
              }).format(new Date(form.date + "T12:00:00"))}</p>
              <p><strong>Hora:</strong> {form.time}</p>
            </div>
            <button className="primary-btn mt-4" onClick={handleNewBooking}>
              Hacer otra reserva
            </button>
          </div>
        </div>
          ...
          <section className="section-card">
            <form onSubmit={handleSubmit} className="page-stack" style={{ gap: 24 }}>
              ...
              <div className="form-grid">
                <div>
                  <label className="block text-sm font-semibold mb-1">
                    Selecciona el Negocio <span className="text-var(--danger)">*</span>
                  </label>
                  <SearchableSelect
                    options={businessOptions}
                    value={form.businessId}
                    onChange={(id) => setForm({ ...form, businessId: id })}
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1">
                    Servicio que deseas reservar <span className="text-var(--danger)">*</span>
                  </label>
                  <input
                    type="text"
                    className="input"
                    placeholder="Ej. Corte de pelo, Consulta general..."
              ...
              <div className="form-grid">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1">
                    Hora <span className="text-var(--danger)">*</span>
                  </label>
                  <input
                    type="time"
                    className="input"
