  ...
  return (
    ...
    >
          ...
          {tab === "register" && (
            <form onSubmit={handleRegister} className="flex flex-col gap-4" autoComplete="off">
              ...
              {registerRole === "cliente" && (
                <div className="flex flex-col gap-1">
                  <label style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--text)" }}>
                    Nombre de usuario <span style={{ fontWeight: 400, color: "var(--text-secondary)", marginLeft: 6 }}>(para iniciar sesion)</span>
                  </label>
      ...
      {isSuccess && successMessage && (
        <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", maxWidth: 400, textAlign: "center", zIndex: 200, padding: "var(--space-6)", background: "var(--surface-solid)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", boxShadow: "var(--shadow-lg)" }}>
          <p style={{ fontSize: "2rem", marginBottom: "var(--space-3)" }}>✓</p>
          <p style={{ fontSize: "var(--text-lg)", color: "var(--text)", fontWeight: 700, marginBottom: "var(--space-2)" }}>{successMessage}</p>
          {isBusinessSuccess && (
            <button
              onClick={() => {
