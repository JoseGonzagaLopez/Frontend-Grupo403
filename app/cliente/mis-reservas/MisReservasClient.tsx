...
/** Una reserva ya ha pasado si su fecha+hora es anterior a ahora */
    ...
    >
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
          <div>
            <label style={{ display: "block", fontWeight: 600, marginBottom: 8, fontSize: "var(--text-sm)" }}>
              Puntuación
            </label>
...
function TarjetaReserva({
  ...
  customerName: string;
  onResenaGuardada: (r: Resena) => void;
}) {
  ...
  return (
    <>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--space-3)" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-1)", flex: 1 }}>
            ...
            <p style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)", margin: 0, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              <CalendarDays size={13} />{dateFormatted}
              <Clock size={13} style={{ marginLeft: 4 }} />{appt.time}
            </p>
            {appt.importe > 0 && (
              <p style={{ fontSize: "var(--text-xs)", color: "var(--text-secondary)", margin: 0 }}>{appt.importe}€</p>
            )}
          </div>
          <span style={{
            background: st.bg, color: st.color,
            padding: "4px 14px", borderRadius: "var(--radius-full)",
            fontSize: "var(--text-sm)", fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0,
          }}>{st.label}</span>
        </div>
        ...
        {/* Botón dejar reseña — solo si pasó y no hay reseña todavía */}
        {pasada && !resena && (
...
export default function MisReservasClient({
  appointments,
  customer,
  resenasByAppointment,
}: {
  appointments: Booking[];
  customer: { Nombre: string; id: number } | null;
  resenasByAppointment: Record<number, Resena>;
}) {
  ...
  if (!customer) {
    return (
      <div className="page-stack">
        <section className="page-hero"><div><h2>Mis reservas</h2><p>Debes iniciar sesión.</p></div></section>
      </div>
    );
  }
  ...
  const pasadas  = sorted.filter((a) => hasPassed(a.date, a.time));

  function handleResenaGuardada(apptId: number, r: Resena) {
    setResenas((prev) => ({ ...prev, [apptId]: r }));
  }
  ...
  return (
    <div className="page-stack">
      <section className="page-hero">
        <div>
          <h2>Mis reservas</h2>
          <p>Consulta y valora tus reservas.</p>
        </div>
      </section>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
            {lista.map((appt) => (
              <TarjetaReserva
                key={appt.id}
                appt={appt}
                resena={resenas[appt.id]}
                customerName={customer.Nombre}
                onResenaGuardada={(r) => handleResenaGuardada(appt.id, r)}
              />
            ))}
          </div>
        )}
