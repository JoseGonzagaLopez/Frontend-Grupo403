...
export default function CustomersClient({ initialCustomers }: { initialCustomers: Customer[] }) {
  ...
  function updateCreateForm<K extends keyof CreateCustomerDto>(key: K, value: CreateCustomerDto[K]) {
    setCreateForm((prev) => ({ ...prev, [key]: value }));
  }
  function updateEditForm<K extends keyof CreateCustomerDto>(key: K, value: CreateCustomerDto[K]) {
  ...
  async function confirmDelete() {
    ...
    setDeletingCustomerId(deleteTargetId); setSuccessMessage(""); setErrorMessage("");
    try {
      ...
      if (hasAppointments) {
        ...
        await Promise.all(customerAppointments.map((appt) => deleteAppointment(appt.id)));
      }
      ...
      <section className="customer-grid">
        {filteredCustomers.map((customer) => {
          const negocios = [...new Set(
            (customer.appointments ?? []).map((a) => a.negocio?.Nombre).filter(Boolean)
          )];
          return (
